...
/// <summary>
/// End-to-end integration test for <see cref="CadsBridge.Worker.Jobs.CtsDeltaScanJob"/>.
/// Expected outcome: exactly 2 SQS messages are enqueued (files 4 and 5).
/// </summary>
[Trait("Dependence", "testcontainers")]
public class CtsDeltaScanJobTests
{
    ...
    [Fact]
    public async Task CtsDeltaScanJob_HappyPath_EnqueuesOnlyValidFilesNotYetCompleted()
    {
        // ── Arrange: start the container with the fake CDS API and the queue consumer
        //   disabled so CtsDeltaScanJob messages stay in the queue long enough for the test
        //   to read them
